...
void ITEMINFO::pb_dline_on_clicked() {
	...
	int cid = model->data(index).toInt();
	int rc = model->rowCount();
	if (cid != rc) {
		index = model->index(r + 1, 1);
		int cid_nl = model->data(index).toInt();
		if (cid == cid_nl) {
			int temp_r = r + 1;
			while (temp_r<rc+1&&model->data(model->index(temp_r,1)).toInt()==cid) {
				index = model->index(temp_r, 2);
				int temp_r_order = model->data(index).toInt();
